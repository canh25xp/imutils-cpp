...
std::vector<std::filesystem::path> imutils::listImages(const std::filesystem::path& basePath, const std::string& contains) {
    return listFiles(basePath, imageTypes, contains);
}

std::vector<std::filesystem::path> imutils::listFiles(const std::filesystem::path& basePath,const std::vector<std::string>& validExts,const std::string& contains) {
