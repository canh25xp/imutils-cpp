...
void imutils::putText(cv::Mat& img, const std::string& text, cv::Point origin, int fontFace, double fontScale, cv::Scalar color, int thickness, int lineType, bool bottomLeftOrigin) {
    ...
    for (auto& line : Mp) {
        ...
        // Draw text
        cv::putText(img, line.second, cv::Point(x, lineY), fontFace, fontScale, color, thickness, lineType, bottomLeftOrigin);
    }
}

void imutils::putCenteredText(cv::Mat& img, const std::string& text, int fontFace, double fontScale, cv::Scalar color, int thickness, int lineType) {
    //Save img dimensions
    int x;
    int y;
